...
# save data for plotting coords - im not gonna save the data because its taking too long - ill just get the summary for the variants
...
#--------------visualize calculated parameters for each cell --------------
...
for fig_num in range(num_figures):
    ...
    for i, parameter in enumerate(current_features):
        ax = plt.subplot(2, 3, i + 1)
        sns.stripplot(data=condens_summary_percell_df, x=x, y=parameter, dodge=True, edgecolor='white', linewidth=1, size=8, alpha=0.4, order=order, ax=ax)
        sns.boxplot(data=condens_summary_percell_df, x=x, y=parameter, palette=['.9'], order=order, ax=ax)
        ax.set_title(parameter, fontsize=12)
        ax.set_xlabel('')
        plt.xticks(rotation=45)
        sns.despine()
    ...
    output_path = f'{output_folder}/puncta-features_percell_raw_fig{fig_num + 1}.png'
    plt.savefig(output_path, bbox_inches='tight', pad_inches=0.1, dpi=300)
    plt.show()
...
# ✅ Build order list with counts included, preserving your order
base_order = ['wt','E66K', 'G67R', 'P50S', 'R68Q', 'R77Q', 'V40G']
order = [f"{cond} (n={cell_counts.get(cond, 0)})" for cond in base_order]

# Use the labeled column for plotting
x = 'variant_labeled'
